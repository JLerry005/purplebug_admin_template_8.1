...
define('PURPLEBUG_TEMPLATE_BASE_URL', plugin_dir_url(dirname(__FILE__)));
define('PURPLEBUG_TEMPLATE_DIR', dirname(__FILE__));
...
function debugginTest(){
    global $wp_query;
    $post_types = get_post_types($wp_query->article);

    // Loop through each post type and output them
...
// add_action('admin_init', 'debugginTest');
    ...
    // if ('article' == $article) {
        ...
        ?>
            <style>
                ...
                .wp-admin .attachments-browser .wp-admin .uploader-inline,
                .wp-admin .attachments-browser.has-load-more .attachments-wrapper,
                .wp-admin .attachments-browser:not(.has-load-more) .attachments {
                    top: 75px !important;
                }
...
function custom_show_user_id_column_content($value, $column_name, $user_id)
{
    ...
    if ('user_id' == $column_name)
        return '<a href="edit.php?author=' . $user_id . '&post_type=article" class="edit"><span aria-hidden="true">' . count(
            get_posts(
                array(
                    'post_type' => 'article',
                    'author' => $user_id,
...
add_filter('login_redirect', function ($redirect_to, $requested_redirect_to, $user) {
    ...
    if (in_array('administrator', $user->roles)) {
        ...
        return $redirect_to = esc_url('/wp-admin/profile.php');
    }
}, 10, 3);
...
/**
 * Categories additional shortcode widget
 */
function categories_widget($attr)
{
    $term_id = shortcode_atts(
        array(
            'term_id' => ''
        ),
        $attr
...
{
    wp_enqueue_media();
    wp_enqueue_script('category-image-upload', get_stylesheet_directory() . '/js/category-image-upload.js', array('jquery'), '1.0', true);
}
...
function list_sub_cat($atts)
{
    ...
    foreach ($parent_categories as $categories) {
        $link = get_category_link($categories->term_id);
        $html .= '<a href="' . $link . '">';
        $html .= '<button class="purple-cat-btn">' . $categories->name . '</button>';
        $html .= '</a>';
    }
    return $html;
}
add_shortcode('list_sub_cat', 'list_sub_cat');

function category_breadcrumb($atts)
{
    $current_term = get_queried_object();
    $html = '';
    if ($current_term->taxonomy == 'category') {
        $ancestors = get_term_parents_list($current_term->term_id, 'category', array('separator' => ' &raquo; ', 'inclusive' => false));
    } elseif ($current_term->taxonomy == 'post_tag') {
        $ancestors = get_the_tag_list('', ' &raquo; ', '');
    }

    if ($ancestors) {
        $html .= '<div class="breadcrumbs">HOME -' . strtoupper($ancestors) . ' ' . strtoupper($current_term->name) . '</div>';
    } else {
        $html .= '<div class="breadcrumbs">HOME -' . strtoupper($current_term->name) . '</div>';
    }
    return $html;
}
add_shortcode('category_breadcrumb', 'category_breadcrumb');